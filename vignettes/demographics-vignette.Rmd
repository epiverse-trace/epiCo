...
-   Estimate the incidence rate of a municipality, department, 
...
7. Generate a treemap of reported occupations to estimate an exposure risk
...
data_event <-  import_data_event(year = 2019,
                                 nombre_event = "dengue")
data_event_clean <- limpiar_data_sivigila(data_event = data_event, year = 2020)
```
...
```
If groups within the incidence object are not within the DIVIPOLA coding for municipalities (level 2) or departments (level 1), or a national estiamtion is intended (level 0), the function will not be able to estimate an incidence rate.
