...
tags_to_analyze <- c("FEC_NOT","COD_PAIS_O", "COD_DPTO_O","COD_MUN_O","COD_DPTO_R","COD_MUN_R","COD_DPTO_N", "COD_MUN_N")
...
for (y in years_to_analyze) {
  temp_data <- sivirep::import_data_event(y, disease_name)
  temp_data$FEC_NOT <- as.character(temp_data$FEC_NOT)
  
  ## Dates cleaning
  temp_data <- disease_name::format_fecha(temp_data,
                                          format_fecha = "%AAAA-%MM-%DD",
                                          nombres_col = "FEC_NOT")
  disease_data <- rbind(
    disease_data,
    dplyr::select(temp_data,tags_to_analyze))
}

## DIVIPOLA codes cleaning and dates handling

disease_data <- sivirep::limpiar_encabezado(disease_data)
disease_data <- sivirep::estandarizar_geo_cods(disease_data)
...
data_event <- dplyr::filter(data_event,.data$epi_year %in% years_to_analyze)
...
## Filtering data for the municipality of interest

data_event <- dplyr::filter(data_event, .data$cod_mun_o == divipola_code)
...
observations <- c(17,23,21,11,25,24,19,17,16,11,23,20,17,24,23,33,53,55,
                  44,36,34,51,41,44,27,32,53,55,55,48,64,69,73,59,74,78,75)

outlier_years <- c(2016,2019)

endemic_channel(incidence_historic = incidence_historic,
