...
ibague_pyramid_2019 <- population_pyramid(ibague_code, year) # Population pyramid (dataframe) for the city of Ibagué in the year 2019 dissagregated by sex
...
ibague_pyramid_2016 <- population_pyramid(ibague_code, year, range = age_range, gender=T, total = FALSE, plot = TRUE)
```
...
describe_ethnicity(demog_data$ethnicity_label)
# Description of ethnicities reported in the consulted dataset
describe_occupation(
...
```{r}
incidence_object <- incidence(dates = data_tolima$fec_not, groups = data_tolima$cod_mun_o, interval = "1 epiweek")
...
age_risk_data <- age_risk(age = data_ibague$edad, population_pyramid = ibague_pyramid_2019, gender = TRUE, plot = TRUE)
```
