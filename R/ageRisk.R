...
ageRisk <- function(cases, populationPyramid, gender = TRUE, plot = FALSE) {
  ...
  if(gender == TRUE){
    ...
    hist_F <- hist(cases_F$Age, breaks = c(0:101), right = FALSE, plot = FALSE)
    
    riskAge_F <- data.frame(Age = pyramid_F$Age,
    ...
    hist_M <- hist(cases_M$Age, breaks = c(0:101), right = FALSE, plot = FALSE)
    
    riskAge_M <- data.frame(Age = pyramid_M$Age,
    ...
    hist_T <- hist(cases$Age, breaks = c(0:101), right = FALSE, plot = FALSE)
    
    riskAge <- data.frame(Age = populationPyramid$Age,
  ...
  if(plot == TRUE){
    ...
    if(gender == TRUE){
      ...
      riskAge_plot <- ggplot(riskAge, aes(x = Age, y = Prob, fill = Gender)) + 
        geom_bar(data = subset(riskAge, Gender == "F"), stat = "identity") + 
        geom_bar(data = subset(riskAge, Gender == "M"), stat = "identity") +
        coord_flip()
