...
endemic_channel <- function(observations, incidence_historic,
  ...
  historic <- as.data.frame(matrix(counts_historic, nrow = length(years),
                                   byrow = TRUE))
  ...
  if (outliers_handling == "included") {
    ...
    if (geom_method == "optimized") {
      handling <- apply(historic, MARGIN = 2, FUN = geom_mean,
                        method = geom_method)
      handling <- as.numeric(handling[1, ])
    } else {
      handling <- as.numeric(apply(historic, MARGIN = 2, FUN = geom_mean,
                                   method = geom_method))
    }
  ...
  if (method == "median") {
    central <- as.numeric(apply(historic, MARGIN = 2,
                                FUN = stats::quantile, p = c(0.5)))
    UL <- as.numeric(apply(historic, MARGIN = 2,
                           FUN = stats::quantile, p = c(0.75)))
    LL <- as.numeric(apply(historic, MARGIN = 2,
  ...
  if (plot == TRUE) {
    endemic_channel_plot <- ggplot2::ggplot(channel_data, 
                                            ggplot2::aes(x = as.numeric(
                                              row.names(channel_data)))) +
      ggplot2::geom_area(ggplot2::aes(y = rep(max(UL) * 1.05, 52), 
                                      fill = "Epidemic")) +
      ggplot2::geom_area(ggplot2::aes(y = UL,
                                      fill = "Warning")) +
      ggplot2::geom_area(ggplot2::aes(y = central,
                                      fill = "Safety")) +
      ggplot2::geom_area(ggplot2::aes(y = LL,
                                      fill = "Success")) +
      ggplot2::geom_vline(xintercept = seq(1, 52, 1),
                          color = "azure2", size = .1) +
      ...
      ggplot2::geom_line(ggplot2::aes(y = LL), linewidth = 1,
                         color = "darkgreen") +
      ggplot2::geom_line(ggplot2::aes(y = obs), linetype = "dashed",
                         linewidth = 0.75) +
