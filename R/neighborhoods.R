...
#' @export
neighborhoods <- function(codes, threshold = 2){
  data("distance_matrix", package = "epiCo")
  distance <- distance_matrix[which(row.names(distance_matrix) %in% codes), 
...
neighborhoods(codes = DIVIPOLA_table$COD_MPIO[1:20])
